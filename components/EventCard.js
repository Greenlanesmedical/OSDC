    ...
    <article className="rounded-2xl overflow-hidden border gold-border shadow-subtle bg-white">
      ...
      <div className="p-5">
        ...
        <div className="flex gap-3 text-sm">
          <a href={`https://www.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(event.title)}&dates=${toICSDate(event.start)}/${toICSDate(event.end)}&details=${encodeURIComponent(event.description||'')}&location=${encodeURIComponent(event.location||'Oxford Street, London')}`} target="_blank" rel="noreferrer" className="underline">Google Calendar</a>
          <a href={`https://outlook.live.com/calendar/0/deeplink/compose?subject=${encodeURIComponent(event.title)}&startdt=${encodeURIComponent(new Date(event.start).toISOString())}&enddt=${encodeURIComponent(new Date(event.end).toISOString())}&body=${encodeURIComponent(event.description||'')}&location=${encodeURIComponent(event.location||'Oxford Street, London')}`} target="_blank" rel="noreferrer" className="underline">Outlook (personal)</a>
          <a href={`https://outlook.office.com/calendar/0/deeplink/compose?subject=${encodeURIComponent(event.title)}&startdt=${encodeURIComponent(new Date(event.start).toISOString())}&enddt=${encodeURIComponent(new Date(event.end).toISOString())}&body=${encodeURIComponent(event.description||'')}&location=${encodeURIComponent(event.location||'Oxford Street, London')}`} target="_blank" rel="noreferrer" className="underline">Outlook (work)</a>
        </div>
